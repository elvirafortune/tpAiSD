...
void binary_quick_sort(int64_t *a , int left , int right , int num) {
    int i = left;
    int j = right;
    if ( right <= left || num < 0)
        return;
    ...
    binary_quick_sort(a, left, j-1, num-1);
    binary_quick_sort(a, j, right, num-1);
}
